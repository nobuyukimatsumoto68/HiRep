...
void polyakov() {
    ...
    for (mu = 1; mu < 4; mu++) {
        ...
        for (x[0] = 0; x[0] < GLB_T; x[0]++) {
            lprintf("FUND_POLYAKOV", 0,
                    "Spatial Polyakov (direction, t, (fund), (adj) )= %d %d ( %1.8e %1.8e I ) , ( %1.8e ) \n", mu, x[0],
                    creal(poly[x[0]] / (GLB_VOL3 * NG) * loc[mu]), cimag(poly[x[0]] / (GLB_VOL3 * NG) * loc[mu]),
        ...
        if (COORD[mu] == np[mu] - 1) {
            ...
            if (mpiret != MPI_SUCCESS) {
                ...
                for (k = 0; k < np[mu] - 1; ++k) {
                    if (status[k].MPI_ERROR != MPI_SUCCESS) {
                        ...
                        lprintf("MPI", 0, "Req [%d] Source [%d] Tag [%] ERROR: %s\n", k, status[k].MPI_SOURCE,
                                status[k].MPI_TAG, mesg);
                    }
                }
            ...
            MPI_Status status;
            {
                ...
                if (mpiret != MPI_SUCCESS) {
                    ...
                    if (status.MPI_ERROR != MPI_SUCCESS) {
                        MPI_Error_string(status.MPI_ERROR, mesg, &mesglen);
                        lprintf("MPI", 0, "Req [%d] Source [%d] Tag [%] ERROR: %s\n", 0, status.MPI_SOURCE, status.MPI_TAG,
                                mesg);
                    }
